...
class UpsamplingNearest1d(nn.Module):
...
class _DenseLayer(nn.Sequential):
    ...
    def __init__(self, in_features, growth_rate, drop_rate=0., bn_size=8,
        ...
        if bottleneck and in_features > bn_size * growth_rate:
            ...
            self.add_module('conv1', nn.Conv1d(in_features, bn_size *
...
class _Transition(nn.Sequential):
    def __init__(self, in_features, out_features, down, bottleneck=True, 
                 drop_rate=0, padding=1, upsample='nearest'):
        ...
        if down:
            ...
            if bottleneck:
                ...
                if upsample is None:
                    self.add_module('convT2', nn.ConvTranspose1d(
...
class DenseED(nn.Module):
    def __init__(self, in_channels, out_channels, blocks, growth_rate=16,
        ...
        self.features.add_module('In_conv', nn.Conv1d(in_channels, init_features // 4, 
                              kernel_size=5, stride=1, padding=2*2, 
                              bias=False, padding_mode='circular'))
        self.features.add_module('In_conv_norm', nn.BatchNorm1d(init_features // 4))
        self.features.add_module('In_conv_relu', nn.ReLU(True))

        self.features.add_module('In_conv2', nn.Conv1d(init_features // 4, init_features // 2, 
                              kernel_size=3, stride=1, padding=1*2, 
                              bias=False, padding_mode='circular'))
        self.features.add_module('In_conv_norm2', nn.BatchNorm1d(init_features // 2))
        self.features.add_module('In_conv_relu2', nn.ReLU(True))

        self.features.add_module('In_conv3', nn.Conv1d(init_features // 2, init_features, 
        ...
        last_trans_up = last_decoding(num_features, out_channels, drop_rate=0.,
                            upsample='nearest')
