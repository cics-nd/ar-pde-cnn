...
class DenseED(nn.Module):
    def __init__(self, in_channels, out_channels, blocks, growth_rate=16,
        ...
        self.features.add_module('In_conv1', nn.Conv2d(in_channels, init_features, 
                              kernel_size=5, stride=1, padding=2*2, 
                              bias=False, padding_mode='circular'))
        self.features.add_module('In_conv_norm1', nn.BatchNorm2d(init_features))
        self.features.add_module('In_conv_relu1', nn.ReLU(True))

        self.features.add_module('In_conv3', nn.Conv2d(init_features, init_features // 2, 
                              kernel_size=3, stride=2, padding=1*2, 
                              bias=False, padding_mode='circular'))

        # Encoding / transition down ================
        # dense block --> encoding --> dense block --> encoding
        num_features = init_features // 2
    ...
    # def forward(self, x, viscosity):
    #     viscosity = viscosity.unsqueeze(-1).unsqueeze(-1).expand_as(x)        
    #     return self.features(torch.cat((x, viscosity), 1))
